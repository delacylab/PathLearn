...
class PaddingConcatenate:
    """
    This class is designed to preprocess incomplete timeseries datasets. Notice that a feature may have no samples in a
    given timestamp. In that case, we impute the no-sample feature with surrogate data, either as a constant or from
    samples in other timestamps.
    ...
    C4. get_numpy()
        ...
        (a) A numpy array encoding self.data_output_dict with dimension of
            (sample size, number of timestamps, number of features).
        ...
        for ts_i, data_i in self.data_input_dict.items():
            ...
            if self.verbose == 1:
                print(f"For the {ts_i}-th timestamp, {len(nan_feats)} features will be imputed by surrogate data "
                      f"using strategy={strategy}.", flush=True)
    ...
    def get_result(self):
        ...
        assert self.data_output_dict != {}, \
            f"Method padding must be called first."

        # Obtain the concatenated output as a 2- or 3-dimensional numpy array
        X_output = np.stack([d.to_numpy() for d in self.data_output_dict.values()], axis=1)

        # Obtain the full feature list
        feat_list = self.feat_union

        # Identify the imputed features
        imputed = [feat + '@' + f'{ts_i}' for ts_i, feat_list in self.nan_feature_dict.items() for feat in feat_list]
